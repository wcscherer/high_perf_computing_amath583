...
void matvec(const Vector& x, Vector& y, long partitions) const {
  ...
  for (size_t i = 0; i < partitions; ++i) {
      ...
      threads.push_back(std::thread(&CSCMatrix::matvec_help, this, std::cref(x), std::ref(y), i * blocksize, (i + 1) * blocksize));
...
void t_matvec_help(const Vector& x, Vector& y, unsigned long begin, unsigned long end) const {
  ...
  for (size_t i = 0; i < partitions; ++i) {
      ...
      threads.push_back(std::thread(&CSCMatrix::t_matvec_help, this, std::cref(x), std::ref(y), i * blocksize, (i + 1) * blocksize));
