...
public:
  ...
  void matvec_help(const Vector& x, Vector& y, unsigned long begin, unsigned long end) const{
  ...
  void matvec(const Vector& x, Vector& y, long partitions) const {
  std::vector<std::thread> threads;
  long blocksize = y.num_rows() / partitions;

  for (size_t i = 0; i < partitions; ++i) {

      threads.push_back(std::thread(&CSRMatrix::matvec_help, this, std::cref(x), std::ref(y), i * blocksize, (i + 1) * blocksize));
...
void t_matvec_help(const Vector& x, Vector& y, unsigned long begin, unsigned long end) const {
for (size_t i = begin; i < end; ++i) {
      for (size_t j = row_indices_[i]; j < row_indices_[i + 1]; ++j) {
  ...
  for (size_t i = 0; i < partitions; ++i) {
      ...
      threads.push_back(std::thread(&CSRMatrix::t_matvec_help, this, std::cref(x), std::ref(y), i * blocksize, (i + 1) * blocksize));
