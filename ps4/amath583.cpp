...
void mult_3(const Matrix& A, const Matrix& B, Matrix& C) {
  ...
  //modifying for 5.2.5 - reordered the outer loop to ii, kk, jj from ii, jj, kk
  size_t blocksize = 32;
...
void mult_ijk(const Matrix& A, const Matrix& B, Matrix& C) {
  ...
  // modify mult_ijk by hoisting C(i,j) - performance on par with mult_trans_1
  for (size_t i = 0; i < C.num_rows(); ++i) {
    for (size_t j = 0; j < C.num_cols(); ++j) {
      double t = C(i,j);
      for (size_t k = 0; k < A.num_cols(); ++k) {
