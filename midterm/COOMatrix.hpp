...
public:
  ...
  void t_matvec(const Vector& x, Vector& y) const {
    /*transposing the matrix exchanges the rows for the columns and the columns for the rows.  The total number of
    non-zero elements stays the same. */
    for (size_t k = 0; k < storage_.size(); ++k) {
  ...
  void matmat(const Matrix& B, Matrix& C) const {
    // go through every element of storage_ and multiply into corresponding element of B
     
    for (size_t m = 0; m < storage_.size(); ++m){ 
      for (size_t j = 0; j < C.num_cols(); ++j){
      
        size_t rA  = row_indices_[m];
        size_t cA  = col_indices_[m];
